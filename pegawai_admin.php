...
<head>
	...
	if (isset($_POST['submit'])) {
		...
		$alam = $_POST['alamat'];
			...
			$up1 = mysqli_query($conn, "UPDATE pegawai SET nama_pegawai='$nama', hari='$hari', jam='$jam', alamat='$alam' WHERE id='$id'");
			echo '<script>
			setTimeout(function() {
				swal({
					title: "Data Diubah",
	...
	if (isset($_POST['submit2'])) {
		...
		$job = $_POST['pekerjaan'];
			...
			$add = mysqli_query($conn, "INSERT INTO pegawai (nama_pegawai, hari, jam, alamat, pekerjaan) VALUES ('$nama', '$hari', '$jam', '$alam', '$job')");
			echo '<script>
				setTimeout(function() {
					swal({
						title: "Berhasil!",
...
<body>
								...
								<div class="card">
									...
									<div class="card-body">
										...
										<td>
											...
											<a class="btn btn-danger btn-action" data-toggle="tooltip" title="Hapus" data-confirm="Hapus Data|Apakah anda ingin menghapus data ini?" data-confirm-yes="window.location.href = 'auth/delete_admin.php?type=pegawai&id=<?php echo $row['id']; ?>'" ;><i class="fas fa-trash"></i></a>
										</td>
										</tr>
									<?php } ?>
									</tbody>
					...
					<div class="modal-body">
						<form action="" method="POST" class="needs-validation" novalidate="">
							...
							<div class="form-group row">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="jam" required="" placeholder="hh.mm-hh.mm">
									<div class="invalid-feedback">
										Mohon data diisi!
									</div>
								</div>
					...
					<div class="modal-body">
						<form action="" method="POST" class="needs-validation" novalidate="">
							...
							<div class="form-group row">
								<label class="col-sm-3 col-form-label">Hari</label>
								<div class="col-sm-9">
									<input type="text" class="form-control" name="hari" required="" id="getHari">
									<div class="invalid-feedback">
										Mohon data diisi!
									</div>
								</div>
							...
							<div class="form-group row">
								<label class="col-sm-3 col-form-label">Jam</label>
								<div class="col-sm-9">
									<input type="text" class="form-control" name="jam" required="" placeholder="hh.mm-hh.mm" id="getJam">
									<div class="invalid-feedback">
										Mohon data diisi!
									</div>
								</div>
	...
	<script>
		$('#editUser').on('show.bs.modal', function(event) {
			...
			modal.find('#getHari').val(hari)
			modal.find('#getJam').val(jam)
